...
function buildGeminiConversation_(project, conversation, newMessage, sourceContext, flowContext, authorizedSelection) {
  ...
  var system = [
    ...
    'HTML PRESENTATION CONTRACT (renderer v2): HTML templates are static and cannot run JavaScript. They can use {{path}}, {{#each array}}...{{else}}...{{/each}}, {{#if path}}...{{else}}...{{/if}}, numeric display helpers and controlled SVG charts. Keep artifact.content as a JSON object with the exact field names and nested arrays required by the selected workflow/template contract. Preserve numbers, booleans and arrays as their proper JSON types. Do not replace arrays with Markdown or HTML strings. Do not invent compliance results, numeric defaults or a template ID. The template catalog identifies available templates; it does not define their complete data schemas. If the workflow does not specify the required structure, ask for the missing contract. Do not put executable HTML or scripts inside artifact.content.',
