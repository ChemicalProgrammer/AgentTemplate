...
Versión de revisión 2.0.0-review.4-hotfix.8-safe.
...
## Hotfix 8 safe sobre 2.0.0-review.4

- Reconstruido directamente desde el hotfix 7 funcional, sin reutilizar archivos del hotfix 8 fallido.
- El selector de modelo se mueve debajo del input del chat y muestra estados de verificación, disponibilidad o error.
- La lista continúa usando el endpoint dinámico existente de hotfix 7: la API key personal y el método `generateContent` determinan qué modelos aparecen.
- Un modelo guardado que ya no figure entre los modelos verificados se sustituye en el selector por una opción disponible, sin presentar el modelo histórico como habilitado.
- Los selectores de agente al crear un proyecto o cambiar el agente cargado reciben una flecha y estados de foco más claros.
- Se oculta la etiqueta redundante del agente dentro del chat; la identidad principal permanece en el encabezado del proyecto.
- Los mensajes nuevos del usuario guardan también el modelo elegido junto con las fuentes y flows seleccionados.
...
- El encabezado del chat queda limitado al título de la conversación y el control de colapso; el modelo se selecciona debajo del input y la edición o borrado permanece junto a cada mensaje o chat correspondiente.
